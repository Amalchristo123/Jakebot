...
async def Force_Sub(bot: Client, message: Message, fileid=None):
    ...
    try:
        ...
        if fileid != None:
            btn.append([InlineKeyboardButton("✅ Refresh", url=f"https://t.me/{temp.U_NAME}?start=file_{fileid}")])
        await bot.send_message(
            chat_id=message.from_user.id,
            text=f"<b>Hey {message.from_user.mention}, Send a Join Request to our channel to get your file !</b>",
